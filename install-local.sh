...
# Check bot installation status (Local Mode - No SSL check)
check_bot_status() {
    if [ -f "/var/www/html/webotconfig/config.php" ]; then
        echo -e "\033[32m✅ Bot is installed (Local Mode)\033[0m"
    else
...
function show_logo() {
    ...
    echo -e "\033[1;34m"
    echo "================================================================================="
    echo "  __          __  ______  ____    ____   _______   _____        _   _ ______ _       "
    echo " |  |        |  ||  ____||  _ \  / __ \ |__   __| |  __ \ /\   | \ | |  ____| |      "
    echo " |  |        |  || |__   | |_) || |  | |   | |    | |__) /  \  |  \| | |__  | |      "
    echo " |  |  /\    |  ||  __|  |  _ < | |  | |   | |    |  ___/ /\ \ | . \ |  __| | |      "
    echo " |  | /  \   |  || |____ | |_) || |__| |   | |    | |  / ____ \| |\  | |____| |____  "
    echo " |__|/    \__|__||______||____/  \____/    |_|    |_| /_/    \_\_| \_|______|______| "
    echo "================================================================================="
    echo -e "\033[0m"
    echo ""
    echo -e "\033[1;36mVersion:\033[0m \033[33m1.0.0 (Local Mode)\033[0m"
    echo -e "\033[1;36mGitHub Repository:\033[0m \033[34mhttps://github.com/MasterALiReza/WeBot\033[0m"
    echo -e "\033[1;36mDeveloper:\033[0m \033[34mMasterALiReza\033[0m"
    echo -e "\033[1;36m⭐️WeBot Panel⭐️: \033[0m \033[34mPowerful VPN Bot Solution (Local Testing)\033[0m"
    echo ""
...
function show_menu() {
    show_logo
    echo -e "\033[1;36m1)\033[0m Install WeBot (Local)"
    echo -e "\033[1;36m2)\033[0m Update WeBot"
    echo -e "\033[1;36m3)\033[0m Remove WeBot"
...
# Install Function (Local Mode)
function install_bot() {
    echo -e "\e[32mInstalling WeBot script (Local Mode)... \033[0m\n"

    echo -e "\033[33m⚠️ Local Mode: SSL and domain configuration will be skipped.\033[0m"
    echo -e "\033[33m⚠️ This installation is for local testing only.\033[0m"
    sleep 2
    ...
    echo 'phpmyadmin phpmyadmin/app-password-confirm password webothipass' | sudo debconf-set-selections
    echo 'phpmyadmin phpmyadmin/mysql/admin-pass password webothipass' | sudo debconf-set-selections
    echo 'phpmyadmin phpmyadmin/mysql/app-pass password webothipass' | sudo debconf-set-selections
    ...
    BOT_DIR="/var/www/html/webotconfig"
    if [ -d "$BOT_DIR" ]; then
    ...
    # Use main branch for local testing (more reliable)
    ZIP_URL="https://github.com/MasterALiReza/WeBot/archive/refs/heads/main.zip"

# Check for version flag
if [[ "$1" == "-v" && "$2" == "beta" ]] || [[ "$1" == "-beta" ]] || [[ "$1" == "-" && "$2" == "beta" ]]; then
    ZIP_URL="https://github.com/MasterALiReza/WeBot/archive/refs/heads/main.zip"
elif [[ "$1" == "-v" && -n "$2" ]]; then
    ZIP_URL="https://github.com/MasterALiReza/WeBot/archive/refs/tags/$2.zip"
fi
    ...
    echo -e "\033[33mDownloading from: $ZIP_URL\033[0m"

    # Check if ZIP_URL is empty
    if [ -z "$ZIP_URL" ]; then
        echo -e "\e[91mError: ZIP_URL is empty. Using fallback URL.\033[0m"
        ZIP_URL="https://github.com/MasterALiReza/WeBot/archive/refs/heads/main.zip"
    fi

    wget -O "$TEMP_DIR/bot.zip" "$ZIP_URL" || {
        echo -e "\e[91mError: Failed to download from $ZIP_URL\033[0m"
        echo -e "\e[91mTrying fallback URL...\033[0m"
        ZIP_URL="https://github.com/MasterALiReza/WeBot/archive/refs/heads/main.zip"
        wget -O "$TEMP_DIR/bot.zip" "$ZIP_URL" || {
    ...
    echo -e "\n\033[33mWeBot config and script have been installed successfully.\033[0m"


wait
if [ ! -d "/root/confwebot" ]; then
    sudo mkdir /root/confwebot || {
        echo -e "\e[91mError: Failed to create /root/confwebot directory.\033[0m"
        exit 1
    }

    sleep 1

    touch /root/confwebot/dbrootwebot.txt || {
        echo -e "\e[91mError: Failed to create dbrootwebot.txt.\033[0m"
        exit 1
    }
    sudo chmod -R 777 /root/confwebot/dbrootwebot.txt || {
        echo -e "\e[91mError: Failed to set permissions for dbrootwebot.txt.\033[0m"
        exit 1
    }
    sleep 1
    ...
    echo "${ASAS}user = 'root';" >> /root/confwebot/dbrootwebot.txt
    echo "${ASAS}pass = '${randomdbpasstxt}';" >> /root/confwebot/dbrootwebot.txt
    echo "${ASAS}path = '${RANDOM_NUMBER}';" >> /root/confwebot/dbrootwebot.txt

    sleep 1

    passs=$(cat /root/confwebot/dbrootwebot.txt | grep '$pass' | cut -d"'" -f2)
    userrr=$(cat /root/confwebot/dbrootwebot.txt | grep '$user' | cut -d"'" -f2)
...
echo " "
    # Local Mode: Skip SSL and domain configuration
    echo -e "\e[32mLocal Mode: Configuring for localhost testing...\033[0m\n"

    # Start Apache2 for local testing
    echo -e "\033[33mStarting Apache2 for local testing...\033[0m"
    sudo systemctl enable apache2 || {
    ...
    ROOT_PASSWORD=$(cat /root/confwebot/dbrootwebot.txt | grep '$pass' | cut -d"'" -f2)
    ROOT_USER="root"
    ...
    if [ $? -eq 0 ]; then
        ...
        if [[ $(mysql -u root -p$ROOT_PASSWORD -e "SHOW DATABASES LIKE 'webot'") ]]; then
            clear
            ...
            file_path="/var/www/html/webotconfig/config.php"
            ...
            echo -e "<?php" >> /var/www/html/webotconfig/config.php
            echo -e "${ASAS}APIKEY = '${YOUR_BOT_TOKEN}';" >> /var/www/html/webotconfig/config.php
            echo -e "${ASAS}usernamedb = '${dbuser}';" >> /var/www/html/webotconfig/config.php
            echo -e "${ASAS}passworddb = '${dbpass}';" >> /var/www/html/webotconfig/config.php
            echo -e "${ASAS}dbname = '${dbname}';" >> /var/www/html/webotconfig/config.php
            echo -e "${ASAS}domainhosts = '${YOUR_DOMAIN}/webotconfig';" >> /var/www/html/webotconfig/config.php
            echo -e "${ASAS}adminnumber = '${YOUR_CHAT_ID}';" >> /var/www/html/webotconfig/config.php
            echo -e "${ASAS}usernamebot = '${YOUR_BOTNAME}';" >> /var/www/html/webotconfig/config.php
            echo -e "${ASAS}secrettoken = '${secrettoken}';" >> /var/www/html/webotconfig/config.php
            echo -e "${ASAS}connect = mysqli_connect('localhost', \$usernamedb, \$passworddb, \$dbname);" >> /var/www/html/webotconfig/config.php
            echo -e "if (${ASAS}connect->connect_error) {" >> /var/www/html/webotconfig/config.php
            echo -e "die(' The connection to the database failed:' . ${ASAS}connect->connect_error);" >> /var/www/html/webotconfig/config.php
            echo -e "}" >> /var/www/html/webotconfig/config.php
            echo -e "mysqli_set_charset(${ASAS}connect, 'utf8mb4');" >> /var/www/html/webotconfig/config.php
...
echo -e "$text_to_save" >> /var/www/html/webotconfig/config.php
            echo -e "?>" >> /var/www/html/webotconfig/config.php

            sleep 1

            # Local Mode: Skip webhook setup
            echo -e "\033[33m⚠️ Local Mode: Webhook setup skipped.\033[0m"
            echo -e "\033[33m⚠️ You can test the bot locally by accessing: http://localhost/webotconfig/\033[0m"
            ...
            url="http://localhost/webotconfig/table.php"
            curl $url || {
            ...
            echo -e "\e[102mLocal Bot URL: http://localhost/webotconfig/\033[0m"
            echo -e "\e[104mDatabase address: http://localhost/phpmyadmin\033[0m"
    ...
    ln -vs /root/install.sh /usr/local/bin/webot

}

function install_bot_with_marzban() {
    # Display warning and confirmation
    echo -e "\033[41m[IMPORTANT WARNING]\033[0m \033[1;33mMarzban panel is detected on your server. Please make sure to backup the Marzban database before installing WeBot.\033[0m"
    read -p "Are you sure you want to install WeBot alongside Marzban? (y/n): " confirm
    ...
    if [ "$DB_TYPE" != "mysql" ]; then
        echo -e "\e[91mError: Your database is $DB_TYPE. To install WeBot, you must use MySQL.\033[0m"
    ...
    # Local Mode: Skip Certbot installation
    echo -e "\e[33m⚠️ Local Mode: Certbot installation skipped.\033[0m"
    ...
    clear
    echo -e "\e[33mConfiguring WeBot database credentials...\033[0m"
    ...
    # Download bot files (use main branch for local testing)
    ZIP_URL="https://github.com/MasterALiReza/WeBot/archive/refs/heads/main.zip"
    if [[ "$1" == "-v" && "$2" == "beta" ]] || [[ "$1" == "-beta" ]] || [[ "$1" == "-" && "$2" == "beta" ]]; then
        ZIP_URL="https://github.com/MasterALiReza/WeBot/archive/refs/heads/main.zip"
    elif [[ "$1" == "-v" && -n "$2" ]]; then
...
EOF
    ...
    # Local Mode: Skip SSL setup for Marzban installation
    echo -e "\e[32mLocal Mode: Configuring for localhost testing (Marzban)...\033[0m\n"

    # Use localhost for local testing
    DOMAIN_NAME="localhost"
    echo -e "\e[92mUsing localhost for local testing\033[0m"
    ...
    YOUR_DOMAIN="localhost"  # Use localhost for local testing
    printf "\e[33m[+] \e[36mUsernamebot: \033[0m"
    read YOUR_BOTNAME
...
${ASAS}dbname = '$dbname';
${ASAS}domainhosts = '$YOUR_DOMAIN/webotconfig';
...
EOF
    ...
    # Local Mode: Skip webhook setup for Marzban installation
    echo -e "\033[33m⚠️ Local Mode: Webhook setup skipped for Marzban installation.\033[0m"
    ...
    TABLE_SETUP_URL="http://localhost/webotconfig/table.php"
    ...
    echo -e "\033[102mLocal Bot URL: http://localhost/webotconfig/\033[0m"
    echo -e "\033[104mDatabase address: http://localhost/phpmyadmin\033[0m"
...
function update_bot() {
    ...
    BOT_DIR="/var/www/html/webotconfig"
    if [ ! -d "$BOT_DIR" ]; then
        echo -e "\e[91mError: WeBot is not installed. Please install it first.\033[0m"
        exit 1
    fi
    ...
    TEMP_DIR="/tmp/webot_update"
    mkdir -p "$TEMP_DIR"

    # Download and extract
    echo -e "\033[33mDownloading update from: $ZIP_URL\033[0m"
    ...
    # Backup config file
    CONFIG_PATH="/var/www/html/webotconfig/config.php"
    TEMP_CONFIG="/root/webot_config_backup.php"
    ...
    # Remove old version
    sudo rm -rf /var/www/html/webotconfig || {
        echo -e "\e[91mFailed to remove old bot files!\033[0m"
        exit 1
    }

    # Move new files
    sudo mkdir -p /var/www/html/webotconfig
    sudo mv "$EXTRACTED_DIR"/* /var/www/html/webotconfig/ || {
    ...
    if [ -f "/var/www/html/webotconfig/install.sh" ]; then
        sudo cp /var/www/html/webotconfig/install.sh /root/install.sh
        echo -e "\n\e[92mCopied latest install.sh to /root/install.sh.\033[0m"
    else
        echo -e "\n\e[91mWarning: install.sh not found in /var/www/html/webotconfig/ after update. Cannot update /root/install.sh.\033[0m"
    fi

    # Set permissions
    sudo chown -R www-data:www-data /var/www/html/webotconfig/
    sudo chmod -R 755 /var/www/html/webotconfig/

    # Run setup script
    URL=$(grep '\$domainhosts' "$CONFIG_PATH" | cut -d"'" -f2)
    curl -s "http://localhost/webotconfig/table.php" || {
    ...
    echo -e "\n\e[92mWeBot updated to latest version successfully!\033[0m"

    # Ensure /root/install.sh is executable and linked
    if [ -f "/root/install.sh" ]; then
        sudo chmod +x /root/install.sh
        sudo ln -vsf /root/install.sh /usr/local/bin/webot
        echo -e "\e[92mEnsured /root/install.sh is executable and 'webot' command is linked.\033[0m"
    else
        echo -e "\e[91mError: /root/install.sh not found after update attempt. Cannot make it executable or link 'webot' command.\033[0m"
    fi
}

# Delete Function
function remove_bot() {
    echo -e "\e[33mStarting WeBot removal process...\033[0m"
    LOG_FILE="/var/log/remove_bot.log"
    echo "Log file: $LOG_FILE" > "$LOG_FILE"

    # Check if WeBot is installed
    BOT_DIR="/var/www/html/webotconfig"
    if [ ! -d "$BOT_DIR" ]; then
        echo -e "\e[31m[ERROR]\033[0m WeBot is not installed (/var/www/html/webotconfig not found)." | tee -a "$LOG_FILE"
    ...
    # User Confirmation
    read -p "Are you sure you want to remove WeBot and its dependencies? (y/n): " choice
    if [[ "$choice" != "y" ]]; then
    ...
    echo "Removing WeBot..." | tee -a "$LOG_FILE"
    ...
    echo -e "\e[92mWeBot, MySQL, and their dependencies have been completely removed.\033[0m" | tee -a "$LOG_FILE"
}

function remove_bot_with_marzban() {
    echo -e "\e[33mRemoving WeBot alongside Marzban...\033[0m" | tee -a "$LOG_FILE"
    ...
    if [ ! -d "$BOT_DIR" ]; then
        ...
        DB_NAME="webot"  # Fallback to default database name
        DB_USER=""
    else
        ...
        if [ -f "$CONFIG_PATH" ]; then
            ...
            if [ -z "$DB_USER" ] || [ -z "$DB_NAME" ]; then
                ...
                DB_NAME="webot"  # Fallback to default
                DB_USER=""
            else
                echo -e "\e[92mFound database credentials: User=$DB_USER, Database=$DB_NAME\033[0m" | tee -a "$LOG_FILE"
            fi
        else
            echo -e "\e[93mWarning: config.php not found at $CONFIG_PATH. Assuming default database name 'webot'.\033[0m" | tee -a "$LOG_FILE"
            DB_NAME="webot"
            DB_USER=""
        fi
    ...
    if [ -n "$DB_USER" ]; then
        ...
        WEBOT_USERS=$(docker exec "$MYSQL_CONTAINER" bash -c "mysql -u '$ROOT_USER' -p'$MYSQL_ROOT_PASSWORD' -e \"SELECT User FROM mysql.user WHERE User NOT IN ('root', 'mysql.infoschema', 'mysql.session', 'mysql.sys', 'marzban');\"" | grep -v "User" | awk '{print $1}')
        if [ -n "$WEBOT_USERS" ]; then
            for user in $WEBOT_USERS; do
    ...
    echo -e "\e[92mWeBot has been removed alongside Marzban. SSL certificates remain intact.\033[0m" | tee -a "$LOG_FILE"
}
...
function auto_backup() {
    ...
    BOT_DIR="/var/www/html/webotconfig"
    if [ ! -d "$BOT_DIR" ]; then
        echo -e "\033[31m[ERROR]\033[0m WeBot is not installed ($BOT_DIR not found)."
    ...
    if check_marzban_installed; then
        ...
        BACKUP_SCRIPT="/root/backup_webot_marzban.sh"
        MYSQL_CONTAINER=$(docker ps -q --filter "name=mysql" --no-trunc)
        if [ -z "$MYSQL_CONTAINER" ]; then
...
EOF
        ...
        BACKUP_SCRIPT="/root/webot_backup.sh"
...
function renew_ssl() {
    echo -e "\033[33m⚠️ Local Mode: SSL renewal is not applicable for local testing.\033[0m"
    echo -e "\033[33m⚠️ This function is disabled in local mode.\033[0m"
}
# Function to Manage Additional Bots (Disabled in Local Mode)
function manage_additional_bots() {
    echo -e "\033[33m⚠️ Local Mode: Additional bot management is not available in local testing mode.\033[0m"
    echo -e "\033[33m⚠️ This feature is disabled for local installations.\033[0m"
    echo -e "\033[33m⚠️ Use the main bot for local testing.\033[0m"
    sleep 2
    show_menu
}
function change_domain() {
    ...
    echo -e "\033[33m⚠️ Local Mode: Domain change is not applicable for local testing.\033[0m"
    echo -e "\033[33m⚠️ This function is disabled in local mode.\033[0m"
    echo -e "\033[33m⚠️ You are using localhost for testing.\033[0m"
}
# Added Function for Installing Additional Bot (Disabled in Local Mode)
function install_additional_bot() {
    echo -e "\033[33m⚠️ Local Mode: Additional bot installation is not available.\033[0m"
    echo -e "\033[33m⚠️ This feature is disabled for local testing.\033[0m"

    # Check for root credentials file
    ROOT_CREDENTIALS_FILE="/root/confwebot/dbrootwebot.txt"
...
EOF"
    ...
    git clone https://github.com/MasterALiReza/WeBot.git "$BOT_DIR" || {
...
EOF
    ...
    # Output Bot Information
    echo -e "\033[32mBot installed successfully!\033[0m"
    echo -e "\033[102mLocal Bot URL: http://localhost/$BOT_NAME/\033[0m"
    echo -e "\033[104mDatabase address: http://localhost/phpmyadmin\033[0m"
    echo -e "\033[33mDatabase name: \033[36m$DB_NAME\033[0m"
    echo -e "\033[33mDatabase username: \033[36m$DB_USERNAME\033[0m"
    echo -e "\033[33mDatabase password: \033[36m$DB_PASSWORD\033[0m"
...
# Function to Update Additional Bot (Disabled in Local Mode)
function update_additional_bot() {
    echo -e "\033[33m⚠️ Local Mode: Additional bot update is not available.\033[0m"
    echo -e "\033[33m⚠️ This feature is disabled for local testing.\033[0m"

    # List directories in /var/www/html excluding webotconfig
    BOT_DIRS=$(ls -d /var/www/html/*/ 2>/dev/null | grep -v "/var/www/html/webotconfig" | xargs -n 1 basename)
    ...
    if ! git clone https://github.com/MasterALiReza/WeBot.git "$BOT_PATH"; then
...
# Function to Remove Additional Bot (Disabled in Local Mode)
function remove_additional_bot() {
    echo -e "\033[33m⚠️ Local Mode: Additional bot removal is not available.\033[0m"
    echo -e "\033[33m⚠️ This feature is disabled for local testing.\033[0m"
    ...
    #Function to export additional bot database (Disabled in Local Mode)
function export_additional_bot_database() {
    echo -e "\033[33m⚠️ Local Mode: Additional bot database export is not available.\033[0m"
    echo -e "\033[33m⚠️ This feature is disabled for local testing.\033[0m"

    # List all directories in /var/www/html excluding webotconfig
    BOT_DIRS=$(ls -d /var/www/html/*/ 2>/dev/null | grep -v "/var/www/html/webotconfig" | xargs -n 1 basename)
...
#function to import additional bot database (Disabled in Local Mode)
function import_additional_bot_database() {
    echo -e "\033[33m⚠️ Local Mode: Additional bot database import is not available.\033[0m"
    echo -e "\033[33m⚠️ This feature is disabled for local testing.\033[0m"
...
#function to configure backup additional bot (Disabled in Local Mode)
function configure_backup_additional_bot() {
    echo -e "\033[33m⚠️ Local Mode: Additional bot backup configuration is not available.\033[0m"
    echo -e "\033[33m⚠️ This feature is disabled for local testing.\033[0m"
