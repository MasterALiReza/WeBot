...
check_ssl_status() {
    ...
    if [ -f "/var/www/html/webotconfig/config.php" ]; then
        domain=$(grep '^\$domainhosts' "/var/www/html/webotconfig/config.php" | cut -d"'" -f2 | cut -d'/' -f1)
...
function show_logo() {
    ...
    echo -e "\033[1;36m⭐️WeBot Panel⭐️: \033[0m \033[34mPowerful VPN Bot Solution\033[0m"
    echo ""
...
function show_menu() {
    show_logo
    echo -e "\033[1;36m1)\033[0m Install WeBot"
...
function install_bot() {
    echo -e "\e[32mInstalling webot script ... \033[0m\n"
    ...
    ZIP_URL=$(curl -s https://api.github.com/repos/MasterALiReza/WeBot/releases/latest | grep "zipball_url" | cut -d '"' -f 4)
...
while [[ ! "$domainname" =~ ^[a-zA-Z0-9.-]+$ ]]; do
    ...
    PATHS=$(cat /root/confwebot/dbrootwebot.txt | grep '$path' | cut -d"'" -f2)
    sudo ufw allow 80 || {
...
echo -e "$text_to_save" >> /var/www/html/webotconfig/config.php
            ...
            curl -F "url=https://${YOUR_DOMAIN}/webotconfig/index.php" \
     ...
     "https://api.telegram.org/bot${YOUR_BOT_TOKEN}/setWebhook" || {
            ...
            url="https://${YOUR_DOMAIN}/webotconfig/table.php"
            curl $url || {
...
EOF
    ...
    TABLE_SETUP_URL="https://${YOUR_DOMAIN}/webotconfig/table.php"
...
function update_bot() {
    ...
    if [[ "$1" == "-beta" ]] || [[ "$1" == "-v" && "$2" == "beta" ]]; then
        ...
        ZIP_URL=$(curl -s https://api.github.com/repos/MasterALiReza/WeBot/releases/latest | grep "zipball_url" | cut -d '"' -f4)
    fi
...
function manage_additional_bots() {
    # Check if WeBot main bot is installed
    if [ ! -d "/var/www/html/webotconfig" ]; then
        echo -e "\033[31m[ERROR]\033[0m The main WeBot is not installed (/var/www/html/webotconfig not found)."
...
function change_domain() {
    ...
    CONFIG_FILE="/var/www/html/webotconfig/config.php"
    if [ -f "$CONFIG_FILE" ]; then
        sudo cp "$CONFIG_FILE" "$CONFIG_FILE.$(date +%s).bak"

        sudo sed -i "s/\$domainhosts = '.*\/webotconfig';/\$domainhosts = '${new_domain}\/webotconfig';/" "$CONFIG_FILE"
        ...
        curl -s -o /dev/null -F "url=https://${new_domain}/webotconfig/index.php" \
